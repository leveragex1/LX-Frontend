...
function Plans() {
    ...
    useEffect(() => {
        ...
        const fetchUserPlanStatus = async () => {
            try {
                const response = await axios.get(`https://lx-backend-ta8m.onrender.com/api/plans/user-plan/${userId}`);
                if (response.data) {
    ...
    const handlePayment = async () => {
        try {
            const userId = localStorage.getItem('userId');
            const response = await axios.post('https://lx-backend-ta8m.onrender.com/api/plans/purchase', { userId, plan: selectedPlan });
